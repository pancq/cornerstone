          ...
          <g :transform="`translate(${panX}, ${panY}) scale(${scale})`">
            ...
            >
              ...
              <g :transform="`translate(${-nodeWidth / 2 + 56}, ${-nodeHeight / 2 + 12})`">
                <text :x="(nodeWidth - 112) / 2" y="16" text-anchor="middle" class="node-name">{{ node.name }}</text>
                <text v-if="!isCircuitNode(node)" :x="(nodeWidth - 112) / 2" y="34" text-anchor="middle" class="node-ip">{{ (node as DeviceNode).ip_address }}</text>
                <text v-if="isCircuitNode(node) && (node as DeviceNode).bandwidth" :x="(nodeWidth - 112) / 2" y="34" text-anchor="middle" class="node-ip">{{ (node as DeviceNode).bandwidth }} Mbps</text>
                <text v-if="!isCircuitNode(node)" :x="(nodeWidth - 112) / 2" y="52" text-anchor="middle" class="node-status" :class="getStatusClass(node.status)">
